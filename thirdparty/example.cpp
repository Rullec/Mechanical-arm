...
//#pragma comment(lib,"F:\\code\\3dmesh\\lib\\3dmesh.lib")
...
void Optimize_by_HLBFGS(int N, double *init_x, int num_iter, int M, int T, bool with_hessian)
{
	...
	info[4] = num_iter;
	info[6] = T;
	info[7] = with_hessian ? 1 : 0;//// π”√hessianæÿ’Û
	info[10] = 0;
	info[11] = 1;
...
void main()
{
	...
	//std::cout << m_sparse_matrix << std::endl;

	Optimize_by_HLBFGS(N, &x[0], 1000, M, T, true);  // it is LBFGS(M) actually, T is not used
